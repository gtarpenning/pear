...
class NetworkManager:
    ...
    def start_message_server(self):
        ...
        console.print(f"[cyan]Starting TCP message server on port {self.message_port}[/cyan]")
    ...
    def _message_server_loop(self):
        ...
        while self.running and self.message_server:
            try:
                ...
                console.print(f"[green]New peer connection from {address[0]}:{address[1]}[/green]")
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if not data:
                        console.print(f"[yellow]Peer {address[0]}:{address[1]} disconnected[/yellow]")
                        break
                    ...
                    if message['type'] == 'peer_join':
                        ...
                        console.print(f"[green]Peer {peer_info.username}@{peer_info.hostname} joined successfully[/green]")
                    ...
                    console.print("[yellow]Received invalid message format from peer[/yellow]")
                    continue
                    
        except Exception as e:
            console.print(f"[yellow]Peer connection error from {address[0]}:{address[1]}: {e}[/yellow]")
        finally:
            if peer_id:
                console.print(f"[yellow]Cleaning up peer connection: {peer_id}[/yellow]")
    ...
    def connect_to_session(self, session_name: str) -> bool:
        ...
        if session_name not in self.discovered_sessions:
            console.print(f"[yellow]Session '{session_name}' not found, running discovery...[/yellow]")
            self.discover_sessions()
            
            if session_name not in self.discovered_sessions:
                console.print(f"[red]Session '{session_name}' not found after discovery[/red]")
                return False
            
        session_info = self.discovered_sessions[session_name]
        console.print(f"[cyan]Connecting to session: {session_name} at {session_info.host_ip}:{session_info.port}[/cyan]")
        ...
        try:
            ...
            if response.get('status') == 'success':
                ...
                console.print(f"[green]Successfully connected to {session_name}[/green]")
                return True
            else:
        ...
        if self.is_host:
            ...
            if hasattr(self, 'server_connection') and self.server_connection:
                try:
                    ...
                    console.print(f"[yellow]Failed to send message to host: {e}[/yellow]")
            else:
