...
class LLMAgent:
    ...
    WOMAN_NAMES = [
        "Wendy", "Sarah", "Emma", "Olivia", "Sophia", "Isabella", "Ava", "Mia",
        "Charlotte", "Amelia", "Harper", "Evelyn", "Abigail", "Emily", "Ella",
        "Elizabeth", "Camila", "Luna", "Sofia", "Avery", "Grace", "Scarlett",
        "Victoria", "Aria", "Chloe", "Madison", "Eleanor", "Layla", "Penelope"
    ]
    ...
    def activate(self, custom_name: Optional[str] = None) -> str:
        ...
        if litellm is None:
            raise Exception("litellm not installed. Please install it: pip install litellm")
        
        self.name = custom_name or random.choice(self.WOMAN_NAMES)
        self.active = True
        
        # Add introduction message
        self.message_handler.add_system_message(f"🤖 {self.name} (AI Assistant) has joined the chat!")
    ...
    def deactivate(self):
        ...
        if self.active and self.name:
            self.message_handler.add_system_message(f"🤖 {self.name} (AI Assistant) has left the chat.")
    ...
    def _generate_response(self, trigger_message: ChatMessage):
        ...
        try:
            # Get recent context
            recent_messages = self.message_handler.get_messages()[-self.context_window:]
            ...
            self.message_handler.add_message(
                self.name, 
                f"Sorry, I'm having trouble connecting to my brain right now 🤯 ({str(e)[:50]}...)"
            )
    
    def _build_context(self, recent_messages: List[ChatMessage], trigger_message: ChatMessage) -> str:
        ...
        context_parts.append(f"You are {self.name}. Please respond naturally to the conversation.")
    ...
    def set_model(self, model: str):
        ...
        self.model = model
        if self.active:
            self.message_handler.add_system_message(f"🤖 {self.name} is now using model: {model}")
